...
`make test` and by CI. If a file is too long, split it by responsibility. Do not
...
Postgres. `make up` takes a few seconds. Tests that skip because the database is
missing will fail loudly in CI rather than passing silently.
...
```bash
make check      # size guard, linter, and the full test suite
```
