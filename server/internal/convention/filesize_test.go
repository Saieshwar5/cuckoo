...
//
// The rule is enforced as a test, not only in CI, so that a file which has
// grown too long is caught by the same `go test ./...` that everything else
// runs — by a person before they commit, and by an agent before it reports
// success. A long file is a design signal: the package has taken on a second
// job and wants splitting.
func TestFileSizeLimit(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("the checker is a shell script; CI enforces the rule on Linux")
	}
